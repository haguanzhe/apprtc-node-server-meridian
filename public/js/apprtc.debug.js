...
if (navigator.mozGetUserMedia) {
    ...
    MediaStreamTrack.getSources = function (successCb) {
        setTimeout(function () {
            var infos = [{kind: "audio", id: "default", label: "", facing: ""}, {
                kind: "video",
                id: "default",
                label: "",
                facing: ""
...
AppController.prototype.pushCallNavigation_ = function (roomId, roomLink) {
    console.log('pushCallNavigation_.roomLink: ' + roomLink);
    if (!isChromeApp()) {
        window.history.pushState({"roomId": roomId, "roomLink": roomLink}, roomId, roomLink);
    }
};
AppController.prototype.displaySharingInfo_ = function (roomId, roomLink) {
    console.log('displaySharingInfo_.roomLink: ' + roomLink);
    //roomLink = roomLink.replace('', '')
...
Call.prototype.queueCleanupMessages_ = function () {
    apprtc.windowPort.sendMessage({
        ...
        queueMessage: {action: Constants.XHR_ACTION, method: "POST", url: this.getLeaveUrl_(), body: null}
    });
    apprtc.windowPort.sendMessage({
        action: Constants.QUEUEADD_ACTION,
        queueMessage: {
            ...
            data: JSON.stringify({cmd: "send", msg: JSON.stringify({type: "bye"})})
        }
    });
    apprtc.windowPort.sendMessage({
        action: Constants.QUEUEADD_ACTION,
        queueMessage: {action: Constants.XHR_ACTION, method: "DELETE", url: this.channel_.getWssPostUrl(), body: null}
    });
};
...
Call.prototype.connectToRoom_ = function (roomId) {
    ...
    Promise.all([channelPromise, joinPromise]).then(function () {
        ...
        console.log('#2');
        console.log('roomId: ' + this.params_.roomId);
        console.log('clientId: ' + this.params_.clientId);
...
var RoomSelection = function (roomSelectionDiv, uiConstants, recentRoomsKey, setupCompletedCallback) {
    ...
    this.roomIdInput_.value = randomString();
    this.onRoomIdInput_();
